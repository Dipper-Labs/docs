...
## REST APIs
 ...
 After ```dipcli``` rest-server is enabled, You can see all the REST APIs from ```http://localhost:1317/swagger-ui/```  
      ...
      "msg": [
        {
          "type": "dip/IPALClaim",
          "value": {
            "from": "dip1dtpryue8ptzjjm32fwr0a7u5qg6wz02hhnpa30",
            "user_request": {
              "params": {
                "user_address": "dip1dtpryue8ptzjjm32fwr0a7u5qg6wz02hhnpa30",
...
You can create a payload via ```dipcli```. The following is a Tx message of creating a contract without signature.
...
# Res is return of call methods.
```
You can create ```post_data``` and ```payload``` via dipcli. Click [here](./contract.md#How-to-call-smart-contract).